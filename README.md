...
* [Взаимодействие с пользователем](#взаимодействие-с-пользователем)
  * [Серверная интеграции](#серверная-интеграции)
  * [Упрощённой интеграции](#упрощённой-интеграции)
* [Схема работы](#схема-работы)
  * [Инициализирующий запрос](#инициализирующий-запрос)
  * [Выбор покупки](#выбор-покупки)
  * [Выбор платёжной системы](#выбор-платёжной-системы)
...
| Режим cандбокс       |Специальный режим, при котором пользователь может ознакомиться с возможностями приложения                                                        |
...
1. [Серверная интеграция](#серверная-интеграция)
1. [Упрощённая интеграция](#упрощённая-интеграция)
 
...
Авторизация пользователя происходит посредством передачи JSON. Она позволяет добавить возможность взаимодействия с xsolla API в сандбокс режиме. 
...
```
 CreateShop (
			...
			json // string
			isSandbox // bool
			OkCallback, // может не быть параметром
			ErrorCallbck // может не быть параметром
)
```
**Описание параметров**:

| Парметр     |Формат                                          | Описание                                                                               |
|-------------|------------------------------------------------|:--------------------------------------------------------------------------------------:|
| token       | string                                         | Токен                                                                                  | 
| json        | string                                         | JSON                                                                                   | 
| isSandbox   | bool                                           | Необходим для создания сандбокс режима. В поле `url` перед `secure` добавить `sandbox-`|
| OkCallback  | Любой удобный формат 			       | Успешное завершение платежа                                                            |
| ErrorCallbck| Любой удобный формат                           | Ошибка в завершении платежа                                                            | 
...
# Схема работы 
Общая схема взаимодействия [(рис.1)](#общая-схема-работы-с-paystation-api) с Paystation API включает в себя 5 основных шагов:
...
Вся информация, необходимая для определения схемы работы, может быть получена из шага [№1](#инициализирующий-запрос). Далее рассмотрим каждый из этих шагов в отдельности, описание будет состоять из сопровождающего текста и схем, наглядно иллюстрирующих процесс работы сервиса.

 <a id="общая-схема-работы-с-paystation-api"></a>
...
Если был получен корректный ответ от сервера, то в первую очередь будет просматриваться информация о покупке. С помощью этой информации определяется передана ли покупка(`purchase`). Если покупка совершена, то необходимо проверить наличие товара в полях:
* `virtual_currency`, 
* `virtual_items`, 
* `subscription` 
...
Если в полученном ответе поле для товара не пустое, то шаг [№2](#выбор-покупки) пропускается [(рис.1)](#общая-схема-работы-с-paystation-api). Если в полученном ответе поле для платёжной системы не пустое, то шаг [№3](#выбор-платёжной-системы) пропускается [(рис.1)](#общая-схема-работы-с-paystation-api). Схема запроса представлена на [рис.2](#инициализирующий-запрос). 
 
 <a id="инициализирующий-запрос"></a>
...
В зависимости от того, что было получено, будут формироваться следующие пункты меню соответственно [(рис.3)](#открытие-магазина): 
- товары,
- виртуальная валюта,
- подписки.

В зависимости от полученных данных необходимо загрузить одну из категорий и отобразить её для пользователя [(рис.4)](#разделы-магазина). Та же логика работает и при переключении пунктов меню. Далее пользователь может выбрать какой-либо из товаров, и ему предоставляется два способа оплаты покупки: 
* [за реальную валюту](#покупка-за-реальную-валюту),
* [за виртуальную валюту](#покупка-за-виртуальную-валюту).
 
 <a id="открытие-магазина"></a>
 ![рис 3. Шаг №2 открытие магазина](3.png)
 
 <a id="разделы-магазина"></a>
...
Для реализации покупки за виртуальную валюту в первую очередь выполняется запрос `Summary`, в котором будут получены:
* содержимое покупки пользователя,
* метка, которая говорит о возможности пропуска подтверждения от пользователя. 

Если данный запрос выполнен успешно, то происходит переход к следующему запросу `Proceed`. Данный запрос определяет корректность производимой денежной операции. Если он выполняется успешно, то платёж продолжается через Xsolla Balance, в противном случае пользователь получит ошибку. Если была получена ошибка или необходимо подтверждение от пользователя, то мы попадаем на экран подтверждения платежа, на котором пользователю показана:
* покупка пользователя, 
* чекбокс с надписью: “Спрашивать ли подтверждение в следующий раз”,  
* наименование ошибки. 

Подробную схему реализации покупок можно увидеть на [рис.5](#реализация-покупки-товара).
 
 <a id="реализация-покупки-товара"></a>
